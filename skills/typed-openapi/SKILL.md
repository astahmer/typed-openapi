...
- Treat `--default-fetcher` as an editable generated transport. Read its generated file before choosing it. Keep an app's existing transport when it already owns retry, auth, tracing, or environment-specific configuration.
...
`--default-fetcher` writes a second TypeScript file. It replaces path parameters before the fetcher runs, serializes query parameters, adds OpenAPI cookie/header parameters, encodes the request body from `requestFormat`, forwards request overrides, and returns the native `Response`.

The generated file reads `process.env["API_BASE_URL"]` by default and falls back to `https://api.example.com`. Change that line for browser-only environment systems. If OpenAPI defines `securitySchemes`, call `configureFetcher({ getAuth })` before the first request.
