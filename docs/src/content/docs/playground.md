...
:::caution[Playground scope]
The playground showcases core client generation. Files generated by `--tanstack`, `--msw`, and `--default-fetcher` are CLI outputs, so use the paste-ready commands in their dedicated guides.
:::
