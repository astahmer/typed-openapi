...
vi.describe("[JSON Schema -> TypeScript]", () => {
  const toTypeScript = JsonSchema.fold<string>((x) => {
    switch (true) {
      default:
  ...
  vi.test("preserves refs", () => {
    const target = toTypeScript(schema, { canonicalizeRefName });
...
vi.describe("[JSON Schema -> Zod]", () => {
  const toZod = JsonSchema.fold<Zod.ZodType>((x) => {
    switch (true) {
      default:
        return x satisfies never;
      case JsonSchema.isRef(x):
  ...
  vi.test("preserves refs", () => {
    const target = toZod(schema, { canonicalizeRefName });
...
vi.describe("[JSON Schema -> ArkType]", () => {
  const toArkType = JsonSchema.fold<string>((x) => {
    switch (true) {
      default:
  ...
  vi.test("preserves refs", () => {
    const target = toArkType(schema, { canonicalizeRefName });
...
vi.describe("[JSON Schema -> TypeBox]", () => {
  ...
  vi.test("preserves refs", () => {
    const target = toTypeBox(schema, { canonicalizeRefName });
