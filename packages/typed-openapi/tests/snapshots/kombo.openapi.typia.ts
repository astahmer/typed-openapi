...
export const post_PostHrisEmployeesForm = {
  ...
  requestFormat: typia.createIs<"json">(),
  responseFormat: typia.createIs<"json">(),
  parameters: { header: ((input: unknown): input is { "X-Integration-Id": string } => typia.createIs<{ "X-Integration-Id": string }>()(input) && input !== null && typeof input === "object" && Object.keys(input).every((key) => ["X-Integration-Id"].includes(key))), body: isPostHrisEmployeesFormRequestBody },
...
export const post_PostAiApplyPostingsPostingIdInquire = {
  ...
  responses: { 200: isPostAiApplyPostingsPostingIdInquirePositiveResponse, default: typia.createIs<({ status: "error", error: ({ code: (("PLATFORM.RATE_LIMIT_EXCEEDED" | "PLATFORM.CONCURRENCY_LIMIT_EXCEEDED" | "PLATFORM.INTEGRATION_NOT_FOUND" | "PLATFORM.INPUT_INVALID" | "PLATFORM.UNKNOWN_ERROR" | "PLATFORM.IP_NOT_WHITELISTED" | "PLATFORM.AUTHENTICATION_INVALID" | "PLATFORM.TASK_TIMED_OUT" | "REMOTE.SERVICE_UNAVAILABLE" | "REMOTE.RATE_LIMIT_EXCEEDED" | "REMOTE.INPUT_INVALID" | "REMOTE.UNKNOWN_HTTP_ERROR" | "AI_APPLY.JOB_FEED_IMPORT_ALREADY_RUNNING" | "AI_APPLY.JOB_FEED_IMPORT_TIMED_OUT") | null), title: (string | null), message: string, log_url: (string | null) } & Record<string, unknown>) } & Record<string, unknown>)>() },
};
