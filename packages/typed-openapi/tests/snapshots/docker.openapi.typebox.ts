...
type InferSchemaValue<T> = T extends import("@sinclair/typebox").TSchema ? import("@sinclair/typebox").Static<T> : T extends object ? { [K in keyof T as undefined extends InferSchemaValue<T[K]> ? never : K]: InferSchemaValue<T[K]> } & { [K in keyof T as undefined extends InferSchemaValue<T[K]> ? K : never]?: Exclude<InferSchemaValue<T[K]>, undefined> } : T;
