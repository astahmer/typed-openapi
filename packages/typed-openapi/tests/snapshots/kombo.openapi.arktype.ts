...
const __schemas = type.module({
  ...
  Schema6: "Schema4[]",
  Schema4: [type("string"), "|", [type("number"), "|", [type("boolean"), "|", ["Schema5", "|", "Schema6"]]]].narrow((data) => [type("string").allows(data), type("number").allows(data), type("boolean").allows(data), "Schema5".allows(data), "Schema6".allows(data)].filter(Boolean).length === 1),
  ...
  PostAiApplyApplyRequestBody: { submission_token: type("string"), candidate_email: type("string.email"), "query_params?": { "[string]": type("string") }, screening_question_answers: type({ question_id: type("string"), answer: [type("string"), "|", [type("string").array(), "|", [type("number"), "|", [type("boolean"), "|", { name: type("string"), content_type: type("string"), data: type("string") }]]]].narrow((data) => [type("string").allows(data), type("string").array().allows(data), type("number").allows(data), type("boolean").allows(data), type({ name: type("string"), content_type: type("string"), data: type("string") }).allows(data)].filter(Boolean).length === 1) }).array(), "additional_clicks?": type("0 <= number.integer <= 30"), "additional_clicks_scatter_duration?": type("number.integer >= 1") },
