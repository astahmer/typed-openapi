...
const __schemas = type.module({
  ...
  PutAtsApplicationsApplicationIdStageRequestBody: { stage_id: type("string"), "remote_fields?": type({ "workday?": { "Workflow_Step_ID?": type("string"), "Step_Type?": type.enumerated("Next_Step_Reference", "Disposition_Step_Reference") } }).onUndeclaredKey("ignore").and(type({ "greenhouse?": { "post_headers?": { "On-Behalf-Of?": type("string").or(type("null")) } }, "workable?": { "on_behalf_of_user_remote_id?": type("string") } }).onUndeclaredKey("ignore")).onUndeclaredKey("reject") },
  ...
  PostAtsApplicationsApplicationIdResultLinksRequestBody: { label: type("string"), url: type("string.url"), "details?": { custom_field_name_prefix: type("string"), attributes: type({ key: type("string"), value: type("string") }).array() }, "remote_fields?": type({ "icims?": { "assessment_package_id?": type("string") }, "oracle?": { "override_document_category?": type.enumerated("IRC_CANDIDATE_RESUME", "IRC_CANDIDATE_COVERLETTER", "MISC", "IRC_INTERNAL"), "multi_post_to_all_current_applications?": type("boolean") } }).onUndeclaredKey("ignore").and(type({ "greenhouse?": { "post_headers?": { "On-Behalf-Of?": type("string").or(type("null")) } }, "workable?": { "on_behalf_of_user_remote_id?": type("string") } }).onUndeclaredKey("ignore")).onUndeclaredKey("reject") },
  ...
  PostAtsApplicationsApplicationIdRejectRequestBody: { rejection_reason_id: type("string"), "note?": type("string"), "remote_fields?": type({ "greenhouse?": { "rejection_email?": { "[string]": type("unknown") } }, "teamtailor?": { "user_id?": type("string") } }).onUndeclaredKey("ignore").and(type({ "greenhouse?": { "post_headers?": { "On-Behalf-Of?": type("string").or(type("null")) } }, "workable?": { "on_behalf_of_user_remote_id?": type("string") } }).onUndeclaredKey("ignore")).onUndeclaredKey("reject") },
  ...
  PostAiApplyApplyRequestBody: { submission_token: type("string"), candidate_email: type("string.email"), "query_params?": { "[string]": type("string") }, screening_question_answers: type({ question_id: type("string"), answer: [[type("string"), "|", [type("string").array(), "|", [type("number"), "|", [type("boolean"), "|", { name: type("string"), content_type: type("string"), data: type("string") }]]]], ":", (data) => [type("string").allows(data), type("string").array().allows(data), type("number").allows(data), type("boolean").allows(data), type({ name: type("string"), content_type: type("string"), data: type("string") }).onUndeclaredKey("reject").allows(data)].filter(Boolean).length === 1] }).array(), "additional_clicks?": type("0 <= number.integer <= 30"), "additional_clicks_scatter_duration?": type("number.integer >= 1") },
