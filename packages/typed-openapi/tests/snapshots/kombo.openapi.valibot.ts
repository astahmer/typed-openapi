...
export const PostPassthroughToolApiPositiveResponse = v.object({ status: v.literal("success"), data: v.object({ url: v.pipe(v.string(), v.url()), status: v.pipe(v.number(), v.integer()), headers: v.record(v.string(), v.union([v.string(), v.array(v.string())])), data: v.optional(v.unknown()) }), warnings: v.array(v.object({ message: v.string() })) });
...
export const Schema4: v.GenericSchema<Schema4> = v.lazy(() => v.union([v.string(), v.pipe(v.number(), v.minValue(-1.7976931348623157e+308)), v.boolean(), v.lazy(() => Schema5), Schema6]));
