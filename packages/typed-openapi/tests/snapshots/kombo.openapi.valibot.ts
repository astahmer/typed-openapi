...
export const PostAtsApplicationsApplicationIdResultLinksRequestBody = v.strictObject({ label: v.string(), url: v.pipe(v.string(), v.url()), details: v.optional(v.strictObject({ custom_field_name_prefix: v.string(), attributes: v.array(v.strictObject({ key: v.string(), value: v.string() })) })), remote_fields: v.optional(v.intersect([v.partial(v.strictObject({ icims: v.partial(v.strictObject({ assessment_package_id: v.string() })), oracle: v.partial(v.strictObject({ override_document_category: v.picklist(["IRC_CANDIDATE_RESUME", "IRC_CANDIDATE_COVERLETTER", "MISC", "IRC_INTERNAL"]), multi_post_to_all_current_applications: v.boolean() })) })), v.partial(v.strictObject({ greenhouse: v.partial(v.strictObject({ post_headers: v.partial(v.strictObject({ "On-Behalf-Of": v.nullable(v.string()) })) })), workable: v.partial(v.strictObject({ on_behalf_of_user_remote_id: v.string() })) }))])) });
...
export const PostAtsApplicationsApplicationIdRejectRequestBody = v.strictObject({ rejection_reason_id: v.string(), note: v.optional(v.string()), remote_fields: v.optional(v.intersect([v.partial(v.strictObject({ greenhouse: v.partial(v.strictObject({ rejection_email: v.record(v.string(), v.unknown()) })), teamtailor: v.partial(v.strictObject({ user_id: v.string() })) })), v.partial(v.strictObject({ greenhouse: v.partial(v.strictObject({ post_headers: v.partial(v.strictObject({ "On-Behalf-Of": v.nullable(v.string()) })) })), workable: v.partial(v.strictObject({ on_behalf_of_user_remote_id: v.string() })) }))])) });
