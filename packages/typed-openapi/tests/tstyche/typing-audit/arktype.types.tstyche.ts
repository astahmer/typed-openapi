/**
 * arktype generated typing audit.
 * Fixtures: pnpm gen:tstyche-fixtures → tmp/tstyche/typing-audit/arktype/
 */
...
} from "../../../tmp/tstyche/typing-audit/arktype/client.ts";
...
} from "../../../tmp/tstyche/typing-audit/arktype/client-sidecar.ts";

const api = createApiClient({
  fetch: async () => new Response(null, { status: 204 }),
});

describe("arktype generated typing audit", () => {
  ...
  it("preserves recursive and parameterized endpoint inference", async () => {
    ...
    const coercedParams = await api.get<"/params/{id}", get_GetParams>("/params/{id}", {
      path: { id: "1" },
      query: { enabled: "false" },
    });
