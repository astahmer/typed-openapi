/**
 * arktype promise ApiClient — MSW / integration type parity.
 * Fixtures: pnpm gen:tstyche-fixtures → tmp/tstyche/runtimes/arktype/
 */
...
} from "../../../tmp/tstyche/runtimes/arktype/client.ts";
import type { TanstackQueryApiClient } from "../../../tmp/tstyche/runtimes/arktype/tanstack.ts";

const api = createApiClient({
  fetch: async () => new Response("[]", { status: 200, headers: { "content-type": "application/json" } }),
});

describe("arktype promise ApiClient", () => {
  ...
  it("infer response with multiple status / withResponse: undefined", async () => {
    const result = await (null as unknown as InferSuccessData<get_FindPetsByStatus>);
    expect(result).type.toBeAssignableTo<readonly Pet[]>();
  });

  it("infer/narrow response with multiple status / withResponse: true", async () => {
    const result = await api.get("/pet/findByStatus", { withResponse: true } as any);
  ...
  it("infer response with multiple json media types / withResponse: undefined", async () => {
    const result = await (null as unknown as InferSuccessData<get_FindPetsByTags>);
  ...
  it("infer/narrow response with multiple json media types / withResponse: true", async () => {
    const result = await api.get("/pet/findByTags", { withResponse: true } as any);
    expect(result).type.toBeAssignableTo<
  ...
  it("infer/narrow response using api.request / withResponse: true", async () => {
    const result = await api.request("get", "/pet/findByStatus", { withResponse: true } as any);
    expect(result).type.toBeAssignableTo<
      TypedApiResponse<
        {
          200: readonly Pet[];
  ...
  it("TanstackQueryApiClient useQuery data", () => {
    const tanstack = {} as TanstackQueryApiClient;
    const queryWithSomeResponseUnknownData = tanstack.get("/pet/findByStatus", {} as never);
